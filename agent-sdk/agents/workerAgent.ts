...
async function deliverWork(escrowId: string): Promise<void> {
  try {
    ...
    // Generate deliverable using DataAgent
    let deliveryCID: string;
    if (process.env.DATA_AGENT_URL) {
      try {
        console.log('[WorkerAgent] Requesting deliverable generation from DataAgent...');
        const dataAgentResponse = await axios.post(`${process.env.DATA_AGENT_URL}/generate-deliverable`, {
          jobTitle: work.job.title || 'Job',
          jobDescription: work.job.description || '',
          requiredSkills: work.job.requiredSkills || []
        });
        
        if (dataAgentResponse.data.success && dataAgentResponse.data.deliveryCID) {
          deliveryCID = dataAgentResponse.data.deliveryCID;
          console.log('[WorkerAgent] Deliverable generated by DataAgent:', deliveryCID);
        } else {
          throw new Error('DataAgent did not return valid deliverable');
        }
      } catch (dataAgentError: any) {
        console.warn('[WorkerAgent] DataAgent deliverable generation failed:', dataAgentError.message);
        console.log('[WorkerAgent] Falling back to basic deliverable generation...');
        ...
        deliveryCID = await uploadJSON(delivery);
      }
    } else {
      // No DataAgent URL, create basic delivery
      const delivery = {
        escrowId,
        jobId: work.jobId,
        deliveredAt: Date.now(),
